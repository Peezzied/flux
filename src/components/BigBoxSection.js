...
import Reveal, { RevealImage, Reveals } from "./RevealAnimate"
...
export default function BigBoxSection({ background, data, id }) {
    return (
        <LazyLoadComponent>

            <DivBox id={id} container='container relative z-10 grid content-start max-h-full' div=' bg-no-repeat bg-center bg-cover relative after:bigBoxGradient after:bgOverlay' style={{ backgroundImage: `url(${background})` }}>
                <div className="grid text-white grid-cols-1 md:grid-cols-2 fluid-pr py-[7em] gap-y-16 gap-x-10 mx-auto max-w-[63em] px-[2em] h-max">
                    <div className="grid content-start grid-rows-[auto_1fr] gap-y-10 md:gap-y-0 ">
                            ...
                            {data[2].img.map((i, index) => (
                                index === 0 && (
                                    <div className="h-full w-full">
                                        <RevealImage src={i} alt="" srcset="" className='object-cover w-full h-full' variants={variants} />
                                        <div></div>
                                    </div>
                                )
                            ))}
                    ...
                    <div className="grid content-start grid-rows-[auto_auto] gap-y-10 md:gap-y-0 h-max">
                        <div className="max-w-[35rem] space-y-3 md:min-h-[24rem]">
                            <Reveals variants={{
                                start: { y: 100, opacity: 0 },
                                visible: { y: 0, opacity: 1 }
